	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (houseid != null) {
					...
					String dir=request.getServletContext().getRealPath("/housepic");
					for (Part part : parts) {
						if (part.getSubmittedFileName() != null&&part.getSubmittedFileName()!="") {
							...
							File file = new File(dir + File.separator + user.getUser_name()+ File.separator+houseid);
							file.setWritable(true, false);
							file.mkdirs();
							System.out.println(dir + File.separator + user.getUser_name()+ File.separator+houseid);
							if (file.mkdir()) {
								System.out.println("建立了"+file.getPath());
							}else if(file.exists()) {
								System.out.println("已存在"+file.getPath());
							}

							savepath = user.getUser_name() + File.separator +houseid+ File.separator + filename;
							String fianlPath = dir+ File.separator + savepath;
